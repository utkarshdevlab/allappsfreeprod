...
interface TiptapEditorProps {
    ...
    /** Called with the uploaded image URL so the parent can insert it into the editor */
    onInsertImageUrl?: (insertFn: (url: string) => void) => void;
    /** Legacy: called with input change event for file upload — parent calls insertFn from onInsertImageUrl instead */
    onImageUpload?: (e: React.ChangeEvent<HTMLInputElement>) => void;
}

type TabMode = 'visual' | 'html' | 'preview';
...
export default function TiptapEditor({ content, onChange, onInsertImageUrl, onImageUpload }: TiptapEditorProps) {
    const [mode, setMode] = useState<TabMode>('visual');
    const [htmlSource, setHtmlSource] = useState(content);
    // Track whether we're sync'ing to avoid infinite loops
    ...
    // Expose insert function to parent so uploaded image URLs can be inserted properly
    useEffect(() => {
        if (!editor || !onInsertImageUrl) return;
        onInsertImageUrl((url: string) => {
            editor.chain().focus().setImage({ src: url, alt: 'Uploaded image' }).run();
        });
    }, [editor, onInsertImageUrl]);

    // Sync from parent content change (e.g. when loading an existing post)
    useEffect(() => {
        if (!editor) return;
        const currentHtml = editor.getHTML();
        if (content !== currentHtml && content !== htmlSource) {
    ...
    // When switching FROM html tab back to visual, apply the edited source
    const handleTabChange = useCallback((next: TabMode) => {
        ...
        setMode(next);
    }, [mode, editor, htmlSource, onChange]);

    // When editing raw HTML source
    ...
    const wordCount = (mode === 'html' ? htmlSource : editor.getText())
        .replace(/<[^>]+>/g, ' ')
        .trim()
        .split(/\s+/)
    ...
    return (
        <div className="border-2 border-gray-100 rounded-2xl overflow-hidden bg-white focus-within:border-blue-400 focus-within:ring-4 focus-within:ring-blue-50 transition-all">
            ...
            {/* Tab Strip */}
            <div className="flex items-center border-b border-gray-100 bg-gray-50/80 px-4">
                {(['visual', 'html', 'preview'] as TabMode[]).map((tab) => (
                    <button
                        key={tab}
                        type="button"
                        onClick={() => handleTabChange(tab)}
                        className={`px-4 py-3 text-xs font-bold uppercase tracking-widest border-b-2 -mb-px transition-all ${mode === tab
                ...
                <div className="ml-auto text-[10px] text-gray-400 font-mono font-bold tabular-nums py-3">
                    {wordCount} words
                </div>
            </div>
            ...
            {mode === 'visual' && (
                <>
                    {/* Toolbar */}
                    <div className="flex items-center flex-wrap gap-0.5 px-4 py-3 border-b border-gray-100 bg-gray-50/60 backdrop-blur-sm sticky top-0 z-10">
                        {/* Headings dropdown */}
                        <select
                        ...
                        {/* Image upload (legacy prop support) */}
                        {onImageUpload && (
                            <label
                                className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-600 hover:bg-gray-100 hover:text-gray-900 cursor-pointer transition-all"
                                title="Upload Image"
                            >
            ...
            {mode === 'html' && (
                <div className="flex flex-col">
                    <div className="px-4 py-2 bg-gray-900 border-b border-gray-700 flex items-center gap-2">
                        <span className="text-[10px] font-mono text-green-400 font-bold">HTML SOURCE</span>
                        <span className="text-[10px] text-gray-500 ml-auto">Edits apply when switching back to Visual or Preview</span>
                    </div>
                    <textarea
                        value={htmlSource}
                        onChange={(e) => handleHtmlChange(e.target.value)}
                        spellCheck={false}
                        className="w-full min-h-[560px] bg-gray-950 text-green-300 font-mono text-sm p-6 resize-y border-none focus:outline-none focus:ring-0 leading-relaxed"
                        placeholder="<p>Write or paste your HTML here...</p>"
                    />
                    <div className="px-6 py-2 bg-gray-900 flex items-center justify-between border-t border-gray-800">
                        <span className="text-[10px] text-gray-500 font-mono">Raw HTML · {htmlSource.length} chars</span>
                        <button
                            type="button"
                            onClick={() => {
                                setHtmlSource('');
            ...
            {mode === 'preview' && (
                <div className="bg-white">
                    <div className="px-4 py-2 bg-blue-50 border-b border-blue-100 flex items-center gap-2">
                        <span className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">Live Preview</span>
                        <span className="text-[10px] text-blue-400 ml-auto">Rendered exactly as it will appear on the blog</span>
                    </div>
                    {htmlSource.trim() ? (
                        <div
                            className="px-8 py-10 prose prose-gray prose-lg max-w-none
                        ...
                        <div className="px-8 py-24 text-center">
                            ...
                            <p className="text-gray-400 font-medium">Nothing to preview yet. Start writing in Visual or HTML mode.</p>
                        </div>
                    )}
                </div>
            )}
            ...
            <div className="px-6 py-2 border-t border-gray-50 bg-gray-50/50 flex items-center justify-between">
                <span className="text-[10px] text-gray-400 font-mono">
                    {mode === 'visual' ? 'Rich Text · Tiptap' : mode === 'html' ? 'HTML Source Mode' : 'Live Preview'}
                </span>
                <span className="text-[10px] text-gray-400">
                    {htmlSource.length} chars
                </span>
            </div>
        </div>
    );
